...
async fn test_ketama_hashing_with_transforms() {
    ...
    let config_str = KETAMA_CONFIG_TEMPLATE
        ...
        .replace("__BACKEND_1__", &format!("{}:{}", b1.address().ip(), b1.address().port()))
        .replace("__BACKEND_2__", &format!("{}:{}", b2.address().ip(), b2.address().port()))
        .replace("__BACKEND_3__", &format!("{}:{}", b3.address().ip(), b3.address().port()));
    ...
    let winner_addr = find_server_addr_with_request("req-1", &backends).await
    ...
    let winner_2_addr = find_server_addr_with_request("req-2", &backends).await.unwrap();
    
    
    assert_eq!(winner_addr, winner_2_addr, "Transforms failed! UPPER case went to a different server.");
    ...
    let winner_3_addr = find_server_addr_with_request("req-3", &backends).await.unwrap();
    
    
    assert_eq!(winner_addr, winner_3_addr, "Transforms failed! lower case went to a different server.");
