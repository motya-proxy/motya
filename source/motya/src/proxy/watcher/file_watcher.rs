use std::{collections::HashMap, convert::Infallible, path::{Path, PathBuf}, sync::Arc, time::Duration};
...
impl<T: FileConfigLoaderProvider + Clone> ConfigWatcher<T> {
    pub fn new(
        ...
        Self { config, table, watch_entry_path, upstream_factory, config_loader, active_proxies: HashMap::default() }
    }
    ...
    async fn reload(&mut self) -> miette::Result<()> {
        ...
        match self.config_loader.clone().load_entry_point(
                Some(self.watch_entry_path.clone()), 
                &mut new_definitions
            ).await 
        {
            Ok(Some(cfg)) => {        
                ...
                let old_proxies: HashMap<&String, &ProxyConfig> = self.config.basic_proxies
                    .iter()
                    .map(|p| (&p.name, p))
                    .collect();

                let new_proxies: HashMap<&String, &ProxyConfig> = cfg.basic_proxies
                    .iter()
                    .map(|p| (&p.name, p))
                    .collect();
                    ...
                    if let Some(old) = old_proxies.get(name) {
                        ...
                        if old.connectors != new.connectors {
                            if let Some(active_config) = self.active_proxies.get(*name) {
                                ...
                                let upstreams = try_join_all(new.connectors.upstreams.clone()
                                    .into_iter()
...
mod tests {
    ...
    async fn test_watcher_updates_proxies_using_mock() {
        ...
        let resolver = ChainResolver::new(table.clone(), registry.clone()).await.unwrap();
        ...
        let tracked_router = Arc::new(ArcSwap::from_pointee(UpstreamRouter::build(vec![upstream]).unwrap()));
        
        watcher.insert_proxy_state(new_proxy_config.basic_proxies[0].name.clone(), tracked_router.clone());
