...
This is a document that is intended to describe WHAT Motya is, without discussing the details of how
it is or should be implemented.

This document is intended for potential users of the Motya application, with a secondary goal of
...
Motya is a reverse proxy application under design, utilizing the `pingora` reverse proxy engine from
...
The intent is for Motya to act as a binary distribution of the `pingora` engine - providing
...
The remainder of this document describes the intended behavior of the Motya application, including
...
Motya operates by listening to one or more downstream Listener interfaces, accepting connections
from clients.
...
1. Motya MUST accept connections via:
    1. Unix Domain Sockets
    2. TCP Sockets
        1. IPv4
        2. IPv6
2. Motya MUST support the termination of TLS sessions
3. Motya MUST support the specification of TLS algorithms used for a given downstream listener as a
   subset of all supported algorithms
4. Motya MUST support the proxying of:
    1. HTTP0.x/HTTP1.x connections
    2. HTTP2.0 connections
5. Motya MAY support the proxying of:
    1. HTTP3.0 connections.
6. Motya MUST support receiving information from protocols used for pre-proxying, including:
...
Motya operates by making and maintaining connections to one or more upstream services, forwarding
...
1. Motya MUST support a configurable Time To Live (TTL) for DNS lookups
2. Motya MUST support a configurable timeouts for:
    1. Connections
    2. Requests
    3. Successful health checks
3. Motya MUST support pooling of connections, including:
    1. Reuse of TCP sessions for all HTTP versions
    2. Reuse of HTTP2.0 streams for HTTP2.0
4. Motya MUST support health checks of upstream servers
5. Motya MUST support the disabling of use of an upstream server based on failed health checks
5. Motya MUST support load balancing of upstream servers
6. Motya MUST support sending information for protocols used for pre-proxying, including:
    1. v1 and v2 of the PROXY protocol
    2. Cloudflare Spectrum
    3. Akamai X-Forwarded-For (XFF) HTTP header field
7. Motya MUST support the configurable selection of a subset of upstream servers based on HTTP URI
   paths
...
Motya allows for the configurable runtime discovery of upstream servers, in order to dynamically
handle changing sets of upstream servers without requiring a restart or reconfiguration of the
application.

1. Motya MUST support the use of a fixed list of upstream servers
2. Motya MUST support the use of DNS-Service Discovery to provide a list of upstream servers for a
   given service
3. Motya MUST support the use of SRV records to provide a list of upstream servers for a given
   service
4. Motya MUST have a configurable timeout for re-polling poll-based service discovery mechanisms
5. Motya MUST support the use of DNS TTL as timeout value for re-polling poll-based service
   discovery mechanisms

### 2.4 - Request Path Control

Motya allows for configurable behavior modifiers at multiple stages in the request and response
...
1. Motya MUST support modifying or rejecting a connection at any of the following stages:
...
2. Motya MUST support rejecting a connection by returning an error response
3. Motya MUST support CIDR/API range-based filtering allow and deny lists
4. Motya MUST support rate limiting of requests or responses on the basis of one or
   more of the following:
    1. A fixed rate per second
    2. A "burst" rate - allowing for short increases above the fixed rate
5. Motya MUST support application of rate limiting of requests or responses on the per-endpoint
   basis.
6. Motya MUST support removal of HTTP headers on a glob or regex matching basis
7. Motya MUST support addition of fixed HTTP headers to a request
8. Motya MUST support the normalization of request and response headers and bodies, including:
    1. URI normalization
    2. Text encoding
...
Motya allows for configurable observability settings, in order to support the operation and
maintenance of the system.

1. Motya MUST support emitting structured logs at a configurable level
2. Motya MUST provide quantitative metrics and performance counters
3. Motya MUST support push- and pull-based methods of obtaining structured logs, metrics, and
   performance counters
4. Motya MUST support emitting logs and metrics locally to file, stdout, or stderr in a consistently
   structured format.

### 2.6 - Configuration

Motya MUST provide methods for configuration in order to control the behavior of the reverse proxy
application

1. Motya MUST support the configuration of all configurable options via a human editable text file
   (e.g. TOML, YAML).
2. Motya MUST support emitting a configuration file containing all configuration items and default
   configuration settings as a command line option
3. Motya MUST support a subset of configuration options at the command line
4. Motya MUST document all command line configurable options via a help command
5. Motya MUST support a subset of configuration options via environment variables
6. Motya MUST support emitting a list of all configuration options configurable via environment
   variables as a command line option
7. Motya MUST give the following priority to configuration:
...
The following are development practice requirements for initial implementers of Motya.

### 3.1 - Documentation Practices

These requirements relate to the technical documentation of Motya.
...
2. The implementers MUST maintain a separate user-facing documentation, describing usage,
   configuration, installation, and other details and examples.
    1. This MAY be achieved using a tool such as `mdBook`, creating a user facing "Book" for Motya
...
These requirements relate to the performance benchmarking of Motya. No specific performance
...
These requirements document tooling practices expected for the development of Motya.
