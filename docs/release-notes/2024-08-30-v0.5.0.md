...
Motya is a reverse proxy application written in Rust, supported as a [Prossimo Initiative],
built on top of the [Pingora engine] from Cloudflare. Motya began development in Q1 2024,
...
This announcement covers the v0.5.0 release of Motya, the second development release made
public. This release was focused on initial development of core functionality necessary
for evaluation of Motya for early experimental users. This release was led by
...
This second milestone was about extending core functionality of Motya, to move it closer
...
Motya now supports a number of strategies for selecting an upstream server when multiple are
...
In the future, Motya intends to support additional Load Balancing Strategies, and additional
...
Effort was spent in the v0.5.x release to make Motya more pleasant to develop and use for
operators of Motya. These features are outside the typical proxying responsibilities of
Motya, but assist in the usage and operation of Motya as an application.

#### Supporting basic static HTML file serving

Many reverse proxy applications also allow for serving of static files, such as images,
HTML files, CSS files, or other non-dynamic content. Although Motya does not currently
consider this a "core competency", it is a common expectation of users of reverse proxies
such as NGINX.

In v0.5.0, Motya introduced the ability to serve local static files, which was made possible
...
Outside of the planned support for Service Discovery, Motya has generally made the choice
...
To support this, the v0.5.0 release of Motya has added the ability to hot reload Motya,
...
of the [Motya User Manual].

[Hot Reloading Section]: https://onevariable.com/motya-user-manual/reloading.html
...
[KDL Document Language] as the primary configuration language for Motya. Although KDL is
...
name = "Example1"
    ...
    upstream-request-filters = [
        ...
        { kind = "upsert-header", key = "x-proxy-friend", value = "motya" },
    ]

    upstream-response-filters = [
        { kind = "remove-header-key-regex", pattern = ".*ETag.*" },
        { kind = "upsert-header", key = "x-with-love-from", value = "motya" },
    ]
...
At the moment, Motya will continue to support both TOML and KDL configuration, however many new features
...
#### (Unplanned) Initial version of the Motya User Manual

When sharing Motya development progress with people for feedback, it was realized that Motya was missing
documentation oriented towards users and operators of Motya, rather than developers of Motya.

Documentation was added as part of the v0.5.0 release, published as the [Motya User Manual]. This
...
The [Motya User Manual] is temporarily hosted on the OneVariable domain, but will likely move to a
...
Motya now supports configuration of rate limiting, as in the following example:
...
section of the [Motya User Manual].

[rate limiting]: https://onevariable.com/motya-user-manual/config/kdl.html#servicesnamerate-limiting

#### CIDR range-based filtering for rejecting connections

Motya now supports rejection of requests from entire CIDR ranges, specified using slash
...
We'd like to thank [@branlwyd] for their first contribution to Motya in this release,
