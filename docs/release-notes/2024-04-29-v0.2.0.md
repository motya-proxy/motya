...
This announcement covers the v0.2.0 release of Motya, the first development release made
public. This release was intended to set up the first working binary of Motya, and allow
...
This initial milestone was about getting an initial version of the Motya application up and
running. The goal is to achieve a "demo-ready" version of Motya that is possible to
experiment with.
...
This is the first release, so it was necessary to set up the basics of Motya as a console
...
```bash
$ motya --help
2024-04-30T09:11:16.907289Z  INFO motya::config: Parsing CLI options
...
$ motya --validate-configs
2024-04-30T09:11:51.470932Z  INFO motya::config: Parsing CLI options
2024-04-30T09:11:51.471342Z  INFO motya::config: CLI config config=Cli { validate_configs: true, config_toml: None, threads_per_service: None }
2024-04-30T09:11:51.471351Z  INFO motya::config: No TOML file provided
2024-04-30T09:11:51.471353Z  INFO motya::config: Applying CLI options
2024-04-30T09:11:51.471355Z  INFO motya::config: Full configuration config=Config { validate_configs: true, threads_per_service: 8, basic_proxies: [] }
2024-04-30T09:11:51.471376Z  INFO motya: Applying Basic Proxies...
2024-04-30T09:11:51.471379Z  INFO motya: Bootstrapping...
...
As discussed in [the Configuration RFC], Motya will likely need a more extensive configuration file
...
time, e.g. `motya --config-toml path/to/your/config.toml`.
...
[a configuration file]: https://github.com/memorysafety/river/blob/1c443ae88b5249b1bacefd9160e7659448b476d5/source/motya/assets/test-config.toml
...
As shown in the configuration file above, Motya has integrated basic support for the `pingora`
...
check support from `pingora` has been integrated into Motya. It is also possible to terminate
...
to modify or reject proxied requests and responses. As a command line tool, Motya aims to
...
In future releases, closer to a "1.0" release, Motya will need to be packaged for end user usage,
for example in operating system package managers, as a published container image, and other
commonly expected formats.

As Motya is in early developer preview, for now binaries are published as part of the tagged Github
...
1. A `crates-io` release. At the moment, we depend on a slightly patched version of `pingora` in
     ...
     publish future versions on crates-io, allowing for `cargo install motya`.
...
We'd like to thank [@lperdereau] for their first contribution to Motya in this release.
